...
:push-to-overleaf
git checkout overleaf && git add --all . && git commit -S -m "push changes to Overleaf %date% %time%" && git push -u overleaf overleaf:master && git checkout master
GOTO:EOF
...
git checkout overleaf && git push -u origin overleaf && git checkout master
GOTO:EOF
